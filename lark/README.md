...
- 命令以 `FULL` 前缀开头时，分块回复运行过程中的全部 stdout/stderr 输出；
...
- 在命令前加 `FULL` 可以转发全部运行输出，例如：
  `FULL python train.py` 或 `FULL: python train.py`。
  全部输出会拆成约 8000 字符一条的消息，并以约 2 秒的间隔发送，以避免触发飞书消息频率限制。
...
收到消息
  ...
  记录 message_id
  解析可选的 SLICENCE/FULL 前缀
  把命令放入后台执行队列
  立即返回事件处理结果

后台执行命令
  在配置的 Linux 工作目录运行 bash 命令
  如果是普通模式且到达 1 分钟间隔：回复最新 30 行输出
  如果是 FULL 模式：分块回复全部 stdout/stderr
