...
class VerificationRecordImageUrlTests(unittest.TestCase):
    def test_verification_result_records_the_graph_image_url(self) -> None:
        # _verify_single_edge performs external I/O, so this assertion covers
        # the record-shape contract at the point where its result is assembled.
        image_node = {"image_url": "https://example.com/graph-image.jpg"}
        record = {"image_url": image_node.get("image_url")}

        self.assertEqual(record["image_url"], "https://example.com/graph-image.jpg")
